...
const tab = ref(tabs.some(item => item[0] === route.query.tab) ? route.query.tab : 'graph')
...
<template>
  <div class="lab">
    ...
    <div class="lab-layout">
      <nav class="lab-nav" aria-label="职业实验室功能"><button v-for="item in tabs" :key="item[0]" :class="{ active: tab === item[0] }" @click="tab = item[0]; aiOutput = ''"><strong>{{ item[1] }}</strong><small>{{ item[2] }}</small></button></nav>
      <main class="lab-panel">
        <template v-if="tab === 'graph'"><h2>经历证据图谱</h2><p class="lead">逐条对照 JD。线索匹配只表示出现了相似词；点开原文核实，再用面试回答补齐缺口。</p>
          ...
          <div class="field"><label>选择个人简历</label><select v-model="state.resumeId"><option :value="null">未选择</option><option v-for="r in resumes" :key="r.id" :value="r.id">{{ r.title }}</option></select></div>
          <div class="graph-grid"><div class="node-list"><button v-for="node in graph" :key="node.id" :class="{ selected: selectedNode?.id === node.id }" @click="selectedRequirement = node.requirement"><span :class="node.missing ? 'gap' : 'found'">{{ node.chain.answer ? '完整证据链' : node.chain.experience ? '部分关系已确认' : node.evidence.length ? '仅有关键词线索' : '待补证据' }}</span>{{ node.requirement }}</button><p v-if="!graph.length" class="muted">填写岗位要求后，在这里建立对应关系。</p></div>
          <div v-if="selectedNode" class="node-detail"><h3>{{ selectedNode.requirement }}</h3><div class="chain-title">岗位要求 ↓ 经历与简历原文 ↓ 面试回答</div><div class="chain-flow"><span>{{ selectedNode.requirement }}</span><span>{{ selectedNode.chain.experience?.title || '经历证据待确认' }}</span><span>{{ selectedNode.chain.resume?.label || '简历表述待确认' }}</span><span>{{ selectedNode.chain.answer ? '回答原文已确认' : '面试回答待确认' }}</span></div><div class="field"><label>建立可核对关系 · 经历卡</label><select v-model="linkForm.experienceId"><option value="">选择经历</option><option v-for="e in state.experiences" :key="e.id" :value="e.id">{{ e.title }}</option></select><textarea v-model="linkForm.experienceQuote" rows="2" placeholder="从该经历行动或材料来源中复制原句"></textarea></div>
            <div class="field"><label>对应简历段落（可稍后补齐）</label><select v-model="linkForm.componentId"><option value="">尚无对应段落</option><option v-for="c in resumeParts" :key="c.id" :value="c.id">{{ c.label }} · {{ c.text.slice(0, 35) }}</option></select><textarea v-model="linkForm.resumeQuote" rows="2" placeholder="从所选简历段落复制原句"></textarea></div>
            <div class="field"><label>对应面试回答（可稍后补齐）</label><select v-model="linkForm.answerId"><option value="">尚无回答</option><option v-for="a in selectedNode.answers" :key="a.id" :value="a.id">{{ a.question }} · {{ a.answer.slice(0, 30) }}</option></select><textarea v-model="linkForm.answerQuote" rows="2" placeholder="从所选回答复制原句"></textarea></div>
            <button class="primary" @click="saveLink">确认并保存关系</button><button class="inline" @click="clearLink">清除关系</button>
        ...
        <template v-else-if="tab === 'interviewer'">
          ...
          <template v-if="interviewer">
            ...
            <template v-if="interviewer.turns.length >= 3 && !interviewer.decision">
              <h3>作出选择</h3><div class="field"><label for="interviewer-select">更愿意推进哪位候选人？</label><select id="interviewer-select" v-model="interviewerSelected"><option value="">请选择</option><option v-for="kind in interviewer.order" :key="kind" :value="kind">候选人 {{ candidateLabel(interviewer, kind) }}</option></select></div>
